...
object BrokerError {
  ...
  case object ConcurrencyConflict extends BrokerError("Another operation is in progress for this resource")
  case object AsyncRequired
      extends BrokerError("This service plan requires client support for asynchronous service operations")
  case object ServiceIdMismatch
      extends BrokerError("The supplied service_id does not match the service_id of the existing resource")
  case object PlanIdMismatch
      extends BrokerError("The supplied plan_id does not match the plan_id of the existing resource")
  case object PlanChangeNotSupported extends BrokerError("The requested plan cannot be updated from the current plan")
