...
/** `CanEqual` instances for types compared across services.
  *
  * The codegen output cannot be edited, so equality with `-language:strictEquality` has to be enabled from this side
  * via given `CanEqual` instances.
  */

given CanEqual[LastOperationResourceEnums.State, LastOperationResourceEnums.State] = CanEqual.derived
